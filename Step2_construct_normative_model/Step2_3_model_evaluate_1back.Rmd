...
# performance bic
## ------------------- Distribution: ΔBIC -------------------
d1 <- performance_1backAcc_distribution %>%
  drop_na() %>%
...
ggplot(d1, aes(x = distribution, y = delta_BIC_scaled)) +
  ...
  labs(title = "1-back", x = "Distribution", y = ylab_txt) +
  theme_minimal() +
  theme(
...
ggplot(d2, aes(x = bsdf, y = delta_BIC_scaled)) +
  geom_bar(stat = "identity", fill = "#A4C5DF", color = "black", width = 0.7) +
  labs(title = "1-back", x = "Degrees of freedom", y = ylab_txt2) +
  theme_minimal() +
  theme(
    plot.title = element_text(size = 9, hjust = 0.5),
    axis.text.x = element_text(size = 9, angle = 45, hjust = 1, color = "black"),
...
ggsave(paste0(FigureFolder, "/1-back/bsdf_DeltaBIC_1backAcc.pdf"),  width=9, height=7, units="cm")
ggsave(paste0(FigureFolder, "/1-back/bsdf_DeltaBIC_1backAcc.tiff"), width=9, height=7, units="cm")
```
...
# residual plots & Q-Q plots
pdf(paste0(FigureFolder, "/1-back/diagnosis_mod_", str_split_i(as.character(model.1backAcc$mu.formula), "~", 1)[2], ".pdf"), width = 14/2.54, height = 8/2.54, pointsize = 9)
...
pdf(paste0(FigureFolder, "/1-back/diagnosis_owenplot_mod_", str_split_i(as.character(model.1backAcc$mu.formula), "~", 1)[2], ".pdf"), width = 9/2.54, height = 7/2.54, pointsize = 9)
...
dtop(model.1backAcc, par = newpar)
dev.off()
...
```{r bootstrap}
# 1. Oneback Acc
bootstrapdir <- paste0(interfileFolder, "/bootstrap/1-back")
centile_female_boot <- list()
centile_male_boot <- list()
centile_boot <- list()
for (i in 1:10000) {
  centile.tmp <- readRDS(paste0(bootstrapdir, "/1back_centile_bootstrap_", i, ".rds"))
...
ggsave(paste0(FigureFolder, "/1-back/New_bootstrap_P50_95CI_1backAcc_all.tiff"), width=7, height=6, units = "cm")
ggsave(paste0(FigureFolder, "/1-back/New_bootstrap_P50_95CI_1backAcc_all.pdf"), width=7, height=6, units = "cm")
