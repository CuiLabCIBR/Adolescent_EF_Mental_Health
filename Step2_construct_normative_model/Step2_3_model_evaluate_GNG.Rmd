...
# performance bic
## ------------------- Distribution: ΔBIC -------------------
d1 <- performance_GNGd_prime_distribution %>%
  drop_na() %>%
...
ggplot(d1, aes(x = distribution, y = delta_BIC_scaled)) +
  ...
  labs(title = "Go/No-Go", x = "Distribution", y = ylab_txt) +
  theme_minimal() +
  theme(
...
d2 <- performance_GNGd_prime_bsdf %>%
  drop_na() %>%
  filter(converged == TRUE) %>%
  arrange(BIC) %>%                 
  slice_head(n = 10) %>%          
...
ggplot(d2, aes(x = bsdf, y = delta_BIC_scaled)) +
  geom_bar(stat = "identity", fill = "#A4C5DF", color = "black", width = 0.7) +
  labs(title = "Go/No-Go", x = "Degrees of freedom", y = ylab_txt2) +
  theme_minimal() +
  theme(
    plot.title = element_text(size = 9, hjust = 0.5),
    axis.text.x = element_text(size = 9, angle = 45, hjust = 1, color = "black"),
...
ggsave(paste0(FigureFolder, "/Gonogo/bsdf_DeltaBIC_GNGd_prime.pdf"),  width=9, height=7, units="cm")
ggsave(paste0(FigureFolder, "/Gonogo/bsdf_DeltaBIC_GNGd_prime.tiff"), width=9, height=7, units="cm")
```
...
# residual plots & Q-Q plots
pdf(paste0(FigureFolder, "/Gonogo/diagnosis_mod_", str_split_i(as.character(model.GNGd_prime$mu.formula), "~", 1)[2], ".pdf"), width = 14/2.54, height = 8/2.54, pointsize = 9)
...
pdf(paste0(FigureFolder, "/Gonogo/diagnosis_owenplot_mod_", str_split_i(as.character(model.GNGd_prime$mu.formula), "~", 1)[2], ".pdf"), width = 9/2.54, height = 7/2.54, pointsize = 9)
...
dtop(model.GNGd_prime, par = newpar)
dev.off()
...
centile_boot <- list()
for (i in 1:10000) {
  centile.tmp <- readRDS(paste0(bootstrapdir, "/GNGd_centile_bootstrap_", i, ".rds"))
...
ggsave(paste0(FigureFolder, "/Gonogo/bootstrap_P50_95CI_GNGd_prime_all.tiff"), width=7, height=6, units = "cm")
ggsave(paste0(FigureFolder, "/Gonogo/bootstrap_P50_95CI_GNGd_prime_all.pdf"), width=7, height=6, units = "cm")
