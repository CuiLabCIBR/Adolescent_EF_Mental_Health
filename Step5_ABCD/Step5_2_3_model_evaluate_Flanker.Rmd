...
# performance bic
## ------------------- Distribution: ΔBIC -------------------
d1 <- performance_Flanker_distribution %>%
  drop_na() %>%
...
ggplot(d1, aes(x = distribution, y = delta_BIC_scaled)) +
  ...
  labs(title = "Flanker", x = "Distribution", y = ylab_txt) +
  theme_minimal() +
  theme(
...
ggplot(d2, aes(x = bsdf, y = delta_BIC_scaled)) +
  geom_bar(stat = "identity", fill = "#A4C5DF", color = "black", width = 0.7) +
  labs(title = "Flanker", x = "Degrees of freedom", y = ylab_txt2) +
  theme_minimal() +
  theme(
    plot.title = element_text(size = 9, hjust = 0.5),
    axis.text.x = element_text(size = 9, angle = 45, hjust = 1, color = "black"),
...
ggsave(paste0(FigureFolder, "/Flanker/bsdf_DeltaBIC_Flanker.pdf"),  width=9, height=7, units="cm")
ggsave(paste0(FigureFolder, "/Flanker/bsdf_DeltaBIC_Flanker.tiff"), width=9, height=7, units="cm")
```
...
# residual plots & Q-Q plots
pdf(paste0(FigureFolder, "/Flanker/diagnosis_mod_", str_split_i(as.character(model.Flanker$mu.formula), "~", 1)[2], ".pdf"), width = 14/2.54, height = 8/2.54, pointsize = 9)
...
pdf(paste0(FigureFolder, "/Flanker/diagnosis_owenplot_mod_", str_split_i(as.character(model.Flanker$mu.formula), "~", 1)[2], ".pdf"), width = 9/2.54, height = 7/2.54, pointsize = 9)
...
ggplot()+
  ...
  theme_classic()+
  scale_x_continuous(name="Age(years)", limits = c(8.8, 16),breaks = seq(9, 16, by = 2))+
...
ggsave(paste0(FigureFolder, "/Flanker/bootstrap_P50_95CI_Flanker_all.tiff"), width=7, height=6, units = "cm")
ggsave(paste0(FigureFolder, "/Flanker/bootstrap_P50_95CI_Flanker_all.pdf"), width=7, height=6, units = "cm")
```
