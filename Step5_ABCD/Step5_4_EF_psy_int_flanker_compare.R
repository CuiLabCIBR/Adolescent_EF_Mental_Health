...
#source function
source(paste0(functionFolder,"/gamm_varyingcoefficients.R"))
...
psyc_variables_continous <- c( "cbcl_scr_syn_attention_r")
...
# beta table
beta_table <- read_csv("xxxx/corr_EF_psych_continuous.result_ABCD_withbonf.csv")
...
# plot
variable_mapping <- c( "cbcl_scr_syn_attention_r_z" = "Attention")
...
for (model_name in names(interaction_results)) {
  ...
  if (length(slope_value) == 0) {
    message(paste("can not find slope`."))
    slope_value <- beta_table %>%
      filter(parcel == current_variable_raw) %>%
      pull(correstimate)
  }
...
for (model_name in names(interaction_results)) {
  ...
  slope_plot_bar <- ggplot(slope_summary, aes(x = Age_year, y = 1)) +
    geom_col(aes(fill = abs_diff), data = filter(slope_summary, is_significant == TRUE),
             width = 0.1, color = "transparent") +
    scale_fill_gradient(low = "white", high = "#A4C5DF", na.value = "white", guide = "none",
                        limits = global_abs_diff_range) +
    geom_rect(aes(xmin = 8.9, xmax = 15.8, ymin = -0.0, ymax = 1),
              fill = NA, color = "black", size = 0.25) +
    scale_x_continuous(name = " ", limits = c(8.9, 15.8), breaks = seq(9, 16, by = 2)) +
    theme_minimal() +
    theme(
