...
gamm.smooth.predict.interaction <- function(dependentvar, dataname, smoothvar, interest.indep.var, covariates, cluster_obj = NULL){
  set.seed(123)
  ...
  if (bettermodel == "GAMM"){
    ...
    set.seed(123)
    boots.pvalues <- PBmodcomp(lm.model, lm.model.null, nsim=10000, cl = cluster_obj)$test["PBtest","p.value"]
    # interaction effect size
